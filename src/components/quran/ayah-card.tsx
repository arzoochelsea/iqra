...
export function AyahCard({ ayah, shown, active, onPlay }: { ayah: Ayah; shown: { arabic: boolean; transliteration: boolean; translation: boolean }; active: boolean; onPlay: () => void }) {
  return <li className={`ayah-card ${active ? "ayah-active" : ""}`}>
    <div className="flex items-center justify-between"><span className="number-badge" aria-label={`Ayah ${ayah.ayahNumberInSurah}`}>{ayah.ayahNumberInSurah}</span>{ayah.audioUrl ? <AudioPlayer compact src={ayah.audioUrl} label={`ayah ${ayah.ayahNumberInSurah}`} onPlay={onPlay} /> : <ErrorCard title="Audio unavailable" message="Verified recitation is not available for this ayah." />}</div>
