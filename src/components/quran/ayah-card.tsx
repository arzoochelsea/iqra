...
export function AyahCard({ ayah, shown, showWordMeanings, active, onPlay }: { ayah: Ayah; shown: { arabic: boolean; transliteration: boolean; translation: boolean }; showWordMeanings: boolean; active: boolean; onPlay: () => void }) {
  return <li id={`ayah-${ayah.ayahNumberInSurah}`} className={`ayah-card ${active ? "ayah-active" : ""}`}>
    ...
    <WordByWord surahNumber={ayah.surahNumber} ayahNumber={ayah.ayahNumberInSurah} showMeanings={showWordMeanings} />
    <div className="ayah-audio">{ayah.audioUrl ? <AudioPlayer compact src={ayah.audioUrl} label={`ayah ${ayah.ayahNumberInSurah}`} onPlay={onPlay} /> : <ErrorCard title="Audio unavailable" message="Verified recitation is not available for this ayah." />}<AddToJourney variant="ayah" item={{ type: "ayah", surahNumber: ayah.surahNumber, ayahNumber: ayah.ayahNumberInSurah, title: `Ayah ${ayah.ayahNumberInSurah} of Surah ${ayah.surahNumber}` }} /></div>
  </li>;
}
