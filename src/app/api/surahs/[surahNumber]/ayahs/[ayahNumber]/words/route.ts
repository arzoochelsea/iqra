...
export async function GET(_request: Request, { params }: Context) {
  ...
  if (!metadata || !Number.isInteger(ayahNumber) || ayahNumber < 1 || ayahNumber > metadata.ayahCount) {
  ...
  try {
    ...
    const status = error instanceof QuranDataError && error.code === "mismatch" ? 502 : 503;
    return Response.json({ error: "Verified word-by-word meanings could not be loaded safely." }, { status });
  }
}
